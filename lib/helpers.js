...
export const extractServiceFromBookingDetail = (detail) => {
	return `${detail.serviceTitle}, ${stringServiceCategories.at(
		detail.serviceCategoryId
	)}, ${stringSize.at(detail.serviceSize)}, ${stringPlacements.at(detail.servicePlacement)}`;
};
...
export const calculateConfirmedTotal = (bookingDetails) => {
	...
	let total = 0;
	bookingDetails.forEach((a) => {
		if (a.status === BOOKING_DETAIL_STATUS.IN_PROGRESS || a.status === BOOKING_DETAIL_STATUS.CANCELLED) {
			total += a.price;
		}
	});
	return total;
